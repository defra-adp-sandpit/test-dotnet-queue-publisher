...
test-dotnet-queue-publisher Dotnet service.
...
| ApplicationInsights__CloudRole          | Role used for filtering metrics    | no       |           |       | Set to `test-dotnet-queue-publisher-local` in docker compose files    |
...
`docker-compose -p test-dotnet-queue-publisher-test -f docker-compose.yaml -f docker-compose.test.yaml up`
...
`docker-compose -p test-dotnet-queue-publisher-test -f docker-compose.yaml -f docker-compose.test.watch.yaml up`
...
```
variableGroups:
    - test-dotnet-queue-publisher-snd1
    - test-dotnet-queue-publisher-snd2
    - test-dotnet-queue-publisher-snd3
variables:
    - test-dotnet-queue-publisher-APPLICATION-SECRET
```
